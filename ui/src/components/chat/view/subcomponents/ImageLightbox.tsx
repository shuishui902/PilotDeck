...
const ImageLightbox = ({ images, startIndex = 0, onClose }: ImageLightboxProps) => {
  ...
  return createPortal(
    <div
      role="dialog"
      aria-modal="true"
      aria-label={active?.name || t('common:uiText.imagePreview')}
    ...
    >
      <button
        ...
        aria-label={t('common:uiText.closePreview')}
      >
      ...
      {images.length > 1 ? (
        <button
          ...
          aria-label={t('common:uiText.previousImage')}
        >
      ...
      {images.length > 1 ? (
        <button
          ...
          aria-label={t('common:uiText.nextImage')}
        >
          <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
      ...
      >
        <img
          key={active.data}
          src={active.data}
          alt={active.name || t('common:uiText.imagePreview')}
